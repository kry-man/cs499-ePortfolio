// Initialize controller for lodging and read the rooms from the JSON files
//var fs = require('fs');
//var lodgings = JSON.parse(fs.readFileSync('./data/lodgings','utf8'));

const roomsEndpoint = ' http://localhost:3000/api/rooms'
const options = {
    method: 'GET',
    headers: {
        'Accept': 'application/json',
    },
};

const rooms = async function (req, res, next) {
    // console. log('MEALS CONTROLLER BEGIN');
    await fetch(roomsEndpoint, options)
    .then((res) => res.json())
    .then((json) =>{
        let message = null;
        if (!(json instanceof Array)){
            message = "API lookup error";
            json = [];
        } else {
            if(!json.length){
                message = "No rooms exist in our database!";
            }
        }
        res.render('rooms', { title: "Travlr Getaways", lodgings: json, message});
    })
   .catch((err) => res.status(500).send(err.message)); 
};

module.exports = {
    rooms,
};